...
```{r anovadf, warning=FALSE, message=FALSE}
...
```{r descstats, warning=FALSE, message=FALSE}

vrDescStats <- vrData %>%
  group_by(Scenario) %>%
  dplyr::mutate(N = n()) %>%
  dplyr::summarise(n = mean(N),
                   MeanPerformance = mean(OMSScore,na.rm=T),
                   MeanInformationSeeking = mean(filteredActions,na.rm=T),
                   MeanInitialConfidence = mean(t1Confidence,na.rm=T),
                   MeanConfidenceChange = mean(confidenceChange,na.rm=T),
                   MeanInitialDiagnoses = mean(t1numOfDiagnoses,na.rm=T))
...
```{r confidencetime, warning=FALSE, message=FALSE}
...
```{r initialconfidence, warning=FALSE, message=FALSE}
...
```{r subinfoseeking, warning=FALSE, message=FALSE}
...
```{r finalconfidence, warning=FALSE, message=FALSE}
...
```{r pcaclassifier, warning=FALSE, message=FALSE}
