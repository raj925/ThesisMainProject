...
p <- ggplot(stratBreakdown, aes(fill=OverallStrat, y=m_N, x=Condition)) + 
  ...
  scale_x_discrete(limits = c("AD","GBS","MTB","TA","TTP","UC","Overall")) +
...
handle_ties <- function(frequencies) {
  ...
  if (length(ties) > 1) {
    return(c("Hybrid", paste(sort(ties), collapse = ", ")))
  } else {
...
TAData <- TAData %>%
  mutate(DominantStrat = rep(result$most_common_strat, each = 6))

# Remove unnecessary columns
TAData <- select(TAData, -most_common_strat, -tied_strats, -frequency)

TAData$usingDominantStrat <- ifelse(TAData$OverallStrat==TAData$DominantStrat,1,0)

```

```{r, warning=FALSE, message=FALSE}

stratDominantBreakdown <- TAData %>%
  group_by(OverallStrat,usingDominantStrat) %>%
  dplyr::mutate(N = n()) %>%
