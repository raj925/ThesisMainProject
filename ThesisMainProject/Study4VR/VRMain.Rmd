...
se <- c(sd(vrData[vrData$t2InvestigationsComplete==0,]$t2Confidence,na.rm=T)/sqrt(length(vrData[vrData$t2InvestigationsComplete==0,]$t2Confidence)),
        sd(vrData[vrData$t2InvestigationsComplete==1,]$t2Confidence,na.rm=T)/sqrt(length(vrData[vrData$t2InvestigationsComplete==1,]$t2Confidence)))
xb <- c("Investigations Incomplete","Investigations Complete")
yb <- c(mean(vrData[vrData$t2InvestigationsComplete==0,]$t2Confidence,na.rm=T),mean(vrData[vrData$t2InvestigationsComplete==1,]$t2Confidence,na.rm=T))
df <- data.frame("InvestigationCompletion" = xb, "MeanConfidence"= yb)
inf <- ggplot(df) +
  geom_bar( aes(x=InvestigationCompletion, y=MeanConfidence), colour="black", stat="identity", fill="blue", alpha=0.5) +
  geom_errorbar( aes(x=InvestigationCompletion, y=MeanConfidence, ymin=MeanConfidence-se, ymax=MeanConfidence+se), colour="orange", alpha=0.9, size=1.1, width = 0.4) + 
  theme_classic()

print(inf +
        ggtitle("Confidence when Investigations are Complete or Incomplete") +
        labs(x = "Investigation Completeness", y = "Confidence")) 

print(t.test(vrData[vrData$t2InvestigationsComplete==0,]$t2Confidence,
             vrData[vrData$t2InvestigationsComplete==1,]$t2Confidence))
...
confScore <- ggplot(data = vrData, aes(x=t2Confidence, y=OMSScore)) +
  geom_point() +
...
cor.test(vrData$t2Confidence,vrData$OMSScore,method="pearson")


confScore <- ggplot(data = vrData, aes(x=t2Confidence, y=severityChange)) +
  geom_point() +
...
cor.test(vrData$severityChange,vrData$OMSScore,method="pearson")


seniorSev <- ggplot(data = vrData, aes(x=t2Severity, y=t2SeniorReview)) +
  geom_point() +
...
cor.test(vrData$t2Severity,vrData$t2SeniorReview,method="pearson")
...
colnames(anovaDf) <- c("Actions","FActionsOverTime","Score","Scenario","ScenarioGroup","ScenarioNumber","ID")
