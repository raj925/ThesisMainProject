...
for (n in 1:nrow(vrData))
{
  ...
  helpStart <- ifelse(length(helpStart)<1,9999,min(pptActions[pptActions$ACTION.TITLE %in% helpActions,]$ACTION.TIMESTAMP..SECS.AFTER.START.))
  
  vrData$sbarStart[n] <- sbarStart
  vrData$helpStart[n] <- helpStart
  vrData$totalHelpStart[n] <- min(c(sbarStart,helpStart))
  
  treatStart <- unique(actions[which(actions %in% treatmentActions)])
  treatStart <- ifelse(length(treatStart)<1,9999,min(pptActions[pptActions$ACTION.TITLE %in% treatmentActions,]$ACTION.TIMESTAMP..SECS.AFTER.START.))
  
  vrData$treatStart[n] <- treatStart
  ...
  for (a in 1:length(treatmentActions))
  {
    if (treatmentActions[a] %in% actions)
    {
      treatmentActionsVector <- c(treatmentActionsVector,1)
    } else {
      treatmentActionsVector <- c(treatmentActionsVector,0)
    }
  }
  ...
  vrData$treatmentActionsVector[n] <- as.String(treatmentActionsVector)
  ...
  vrData$numOfTreatmentActions[n] <- sum(treatmentActionsVector)
  ...
  vrData$numOfHistoryActionsBeforePause[n] <- sum(unique(actions)[times<300] %in% historyActions)
    vrData$numOfHistoryActionsAfterPause[n] <- sum(unique(actions)[times>300] %in% historyActions)
    vrData$numOfExamActionsBeforePause[n] <- sum(unique(actions)[times<300] %in% examActions)
    vrData$numOfExamActionsAfterPause[n] <- sum(unique(actions)[times>300] %in% examActions)
  vrData$percentageHistory[n] <- vrData$numOfHistoryActionsBeforePause[n]/vrData$numOfHistoryActions[n]
  
    vrData$numOfTestingActionsBeforePause[n] <- sum(unique(actions)[times<300] %in% TestingActions)
  ...
  vrData$percentageTesting[n] <- vrData$numOfTestingActionsAfterPause[n]/vrData$numOfTestingActions[n]
    ...
    vrData$numOfTreatmentActionsBeforePause[n] <- sum(unique(actions)[times<300] %in% treatmentActions)
    vrData$numOfTreatmentActionsAfterPause[n] <- sum(unique(actions)[times>300] %in% treatmentActions)
...
for (x in 1:nrow(vrData))
{
  ...
  examVals <- soughtVals[(length(historyActions)+1):(length(historyActions)+length(examActions))]
  testVals <- soughtVals[(length(historyActions)+length(examActions)+1):length(soughtVals)]
  #vrData$infoValHistory[x] <- mean(historyVals[historyVals!=0],na.rm = T)
  vrData$infoValHistory[x] <- sum(historyVals,na.rm = T)
  vrData$infoValExams[x] <- sum(examVals,na.rm = T)
  vrData$infoValTesting[x] <- sum(testVals,na.rm = T)
  
  pptTimes <- str_split(vrData$actTimesVector[n],"\n")[[1]]
  pptTimes <- as.numeric(pptTimes)
  pptHistTimes <- pptTimes[1:length(historyActions)]
  pptExamTimes <- pptTimes[(length(historyActions)+1):(length(historyActions)+length(examActions))]
  pptTestTimes <- pptTimes[(length(historyActions)+length(examActions)+1):length(pptTimes)]
  ...
  vrData$infoValHistoryAfterPause[x] <- sum(historyVals[pptHistTimes>=300],na.rm = T)
  vrData$infoValExamsAfterPause[x] <- sum(examVals[pptExamTimes>=300],na.rm = T)
  vrData$infoValTestsAfterPause[x] <- sum(testVals[pptTestTimes>=300],na.rm = T)
...
vrDataNoNAs <- vrData[!is.na(vrData$confidenceChange),]
...
model <- lmerTest::lmer(filteredActions ~ t1numOfDiagnoses + (1|Scenario) + (1 | ParticipantID), data=vrData)
summary(model)

model <- lm(t1DiagnosisScore ~ t1numOfDiagnoses*filteredActions, data=vrData)
...
### Predictors of initial confidence
...
histCon <- ggplot(data = vrData, aes(x=t2Confidence, y=t1DiagnosisScore)) +
  geom_point() +
  geom_smooth(method=lm , color=accuracyColour, fill="grey", se=TRUE) +
  theme_classic()

print(histCon + 
        ggtitle("Confidence Against Diagnostic Score") +
      labs(y="Diagnostic Score", x = "Final Confidence"))
...
vrDataHelped <- vrData[vrData$totalHelpStart<9999,]
model <- lmer(totalHelpStart ~ t1Severity + t1Confidence + (1|Scenario) + (1 | ParticipantID), data=vrDataHelped)
summary(model)
