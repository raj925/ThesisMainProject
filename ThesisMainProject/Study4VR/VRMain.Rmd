...
for (n in 1:nrow(vrData))
{
  ...
  actions <- actionsData[actionsData$OMSID == vrData$OMSID[n] & actionsData$SCENARIO == vrData$SCENARIO[n],]$ACTION.TITLE
...
asthData <- vrData[vrData$Scenario=="Asthma",]
dkaData <- vrData[vrData$Scenario=="DKA",]
pneuData <- vrData[vrData$Scenario=="Pneumonia",]
seizData <- vrData[vrData$Scenario=="Seizure",]

corrs <- c(cor.test(asthData$t1numOfDiagnoses,asthData$filteredActions)$estimate,
           cor.test(dkaData$t1numOfDiagnoses,dkaData$filteredActions)$estimate,
           cor.test(pneuData$t1numOfDiagnoses,pneuData$filteredActions)$estimate,
           cor.test(seizData$t1numOfDiagnoses,seizData$filteredActions)$estimate)

corrs

mixedModel <- lmer(filteredActions ~ t1Confidence + OMSScore + Scenario + (1|ParticipantID),data=vrData)
summary(mixedModel)
