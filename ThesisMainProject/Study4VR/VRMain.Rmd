...
model <- lmerTest::lmer(t2Confidence ~ numOfHistoryActionsAfterPause + numOfExamActionsAfterPause + numOfTestingActionsAfterPause + (1|Scenario) + (1 | ParticipantID), data=vrData)
summary(model)

model <- lmerTest::lmer(t1DiagnosisScore ~ numOfHistoryActions + numOfExamActions + numOfTestingActions + (1|Scenario) + (1 | ParticipantID), data=vrData)
summary(model)
